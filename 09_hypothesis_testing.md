...
becomes* *clearer* that evidence against the null hypothesis may come
only with values to the left of 3.6, hence
...
the samples would be the nm=1000 metal songs and the *nf=1000* deep
house songs.
...
In our case, how will the difference in sample proportions *ˆpm−ˆpf*
vary due to sampling under H0? Recall from
...
![](09_hypothesis_testing_files/figure-commonmark/unnamed-chunk-3-1.png)
...
![](09_hypothesis_testing_files/figure-commonmark/unnamed-chunk-4-1.png)

##### !clarity

9.4.1

Going back to the *pennies* example from Chapter 8, say we wanted to
test if the mean weight of all chocolate-covered almonds was equal to
3.5 grams or not.
...
> Why does the following code produce an error? In other words, what
> about the response and predictor variables makes this not a possible
...
```
    ...
    Error in `calculate()`:
    ! A difference in means is not well-defined for a dichotomous
      categorical response variable (popular_or_not) and a dichotomous categorical
      explanatory variable (track_genre).

This code gives an error because we are asking it to calculate means,
but the variables are binary, so there is no mean but only a proportion
to be calculated. Which is technically the mean of 1s and 0s, but it
probably makes a difference in the hypothesize step and such.

#### LC9.2

> Why are we relatively confident that the distributions of the sample
> proportions will be good approximations of the population
> distributions of popularity proportions for the two genres?

We are relatively confident in our distribution of sample proportions
reflecting the population’s distribution of sample proportions because
our sample is a random sample of that population and as we repeat the
sampling sufficiently often we will reach the point at which the
mechanic of the central limit theorem comes into play; that is, if the
sampling size is either large enough, or the sample is repeated often
enought we will the resulting distribution of sample statistics will
follow a normal distribution approximating that of the population.
...
> if a statistical difference existed between the popularity rate of
> metal and deep house for the Spotify example.
...
defined the null hypotheses and simulated data that would arise under
this hypotheses. Then we related the observed value with those from the
simulated data so to observe how likely the observed value would be
